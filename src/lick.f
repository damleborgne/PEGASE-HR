...
!----------------------------------------------------------------------
      ...
      integer i,error,il1,nlambda,il2,j,age,ntimes,Lfits,istat,k,nindex
      integer nlines
      integer hdutype
      character*10 iindices(nmaxlick),uniindice
      double precision Lambda(nmaxlambda),Flux(nmaxlambda),l1,l2,fm,lm
      ...
      character*280     nom_spectra_in,nom_spectra_out,nom_ages,nom_ind
...
!     if interactive mode, ask for file(s)
     ...
     &              //' of the indices (0. for no emission line) :'
            ...
            nom_ind=PEG_ROOT//'data/user_defined/index_table.dat'
            open (30,file=nom_ind,status='old')
            read (30,'(i5)') nindex
            close (30)
            
*     open and reads nlambda,ntimes
            call fits_spec_open_rw(nom_spectra_in,
     ...
     &              //' is too low to measure Lick indices'
            ...
            if (istat.eq.0) then
               ...
               do j=1,ntimes
                  ...
                  call read_spectra_fits(Lfits,j,Lambda,nlambda,Flux,
...
c     find first point in Lambda just after Lambdalines(j)
               icenter=1
               do while (Lambda(icenter).lt.Lambdalines(j)) 
                  icenter=icenter+1
               enddo
c     find first point for the sum of the gaussian
               ileft=1
               do while (Lambda(ileft).lt.Lambdalines(j)-3.*FWHM) 
                  ileft=ileft+1
               enddo
c     find last point for the sum of the gaussian
               iright=1
...
C     **************************************************
      subroutine fits_spec_open_rw(filename,Lfits,nw,nwl,nt,resol,istat)
...
C     **************************************************
     ...
     &     indicesunits,nindices,FWHM)
      ...
      character*16  ttype(nmaxlick),untype
      character*16  tform(nmaxlick)
      character*16  tunit(nmaxlick)
      double precision   indices(nmaxlick)
      real          unindice
      character*(*) iindices(nmaxlick)
...
*                   create LICK HDU               
     ...
     s        'ETS_LICK',0, istat) !create binary extension
...
C     **************************************************
      ...
      integer             nlambda,jres,nres,i,j,pri
      double precision    Lambda(nmaxlambda),Flux(nmaxlambda)
      double precision    Flux_red(nmaxlambda)
      double precision    meanlambda,maxsigma,delta,lambda0,fluxj
     ...
     &        (lambda_lick_res(jres+1)-lambda_lick_res(jres))
      enddo
c ---------------------------------------------------------------------
c     Convolution by g=1/sqrt(2*pi*sigma^2) * exp(-r^2/(2*sigma^2))
c      we have int(g) = 1.0
      do i=1,nlambda
         Flux_red(i)=Flux(i)
      enddo
...
c     computes the flux interpolated at lambda0
...
C     Reads a pegase ASCII spectrum      

      implicit none
      include 'peg_include.f'

      character*280        fichier,header
...
C     Computes the Lick index of line # 'number' described in the file
c     'fichier_ind'.
      ...
      character*280     fichier_ind,header
      character*(*)     iindex
      double precision lb1,lb2,lr1,lr2,lp1,lp2,lbm,lrm,fbm,frm
      
c     Reading the index file ----------------------------------------
...
C     computes flux through passband defined by wavelengths (lp1,lp2)
c     continuum is defined by mean blue and red fluxes (fbm,frm)
c     and by mean blue and red wavelengths (lbm,lrm)
c     Output is fp (flux through pass band) = \int_lp1^lp2 {1-F/Fc}

      implicit none
      include 'peg_include.f'

      double precision Lambda(nmaxlambda),Flux(nmaxlambda)
      double precision Flux_integ(nmaxlambda)
      double precision lbm,lrm,fbm,frm,lp1,lp2,fp,slope,continu
      ...
      if (error.eq.0) then
         ...
         do i=ilp1,ilp2
            ...
            if (continuum_red(i-ilp1+1).eq.0.) then
               error=4
c               write(*,*) 'error : continuum=0!'
c               write(*,*)  'slope=',slope
c               write(*,*)  'fbm,frm=',fbm,frm
            endif
         enddo
      endif
...
C     computes mean flux between l1 (lambda1) and l2 (lambda2). 
C     Output is fm (fluxmean) and lm (lambda mean)
