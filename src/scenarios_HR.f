***** This code is aimed to prepare the input file to "spectra.f".
      ...
      character*280 filescenarios,fileSSPs,filespectra,fileSFR
      character*72 answerneb,answerZ,char_nfiles,answerZdef
      character*72 answernebdef
      character*72 infall,infalldef
      character*280 filespectradef
      character*72 answerwind,answerwinddef
...
!     added by DLB
      integer status
      character*(50) stellib
      character*280 fileSSPsdef
      integer str_length
...
***** Default values
      ...
      Zsteldef=0.02
      fsubdef=0.
      fileSSPsdef=PEG_ROOT//'data/isochrones/Salp_default_SSPs.dat'
...
***** filespectra
     ...
     $        ' ')-1)//'.fits'
         write(*,*) 'Name of the output file ',
     $        '(type "end" to stop)?'
         write(*,*) 'Default: ',filespectradef(1:
     $        index(filespectradef,' '))
         read(*,'(a)') filespectra
         if (filespectra.eq.'END'.or.filespectra.eq.'End') 
     $        filespectra='end'
      end do
